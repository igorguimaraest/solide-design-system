...
A confirmação específica dos estados encontrou uma pendência não coberta pela suíte oficial: em Checkbox, Radio e Switch selecionados, o rótulo entra corretamente em `:active`, porém a cor computada permanece em `--sld-control-selected-hover-bg`; `--sld-control-selected-active-bg` não vence a regra de hover. Os estados selecionado, não selecionado/desligado, hover, foco e disabled continuam definidos por seus papéis próprios, e a implementação não depende de `--sld-action-primary-*`, mas VC-02 permanece **implementada e aprovada visualmente, sem confirmação técnica final** até corrigir e testar a precedência de `active`.
...
**Próximo passo exato:** corrigir a precedência `active` × `hover` nos três controles, adicionar a asserção correspondente à suíte oficial e repetir `npm run test:ui` em 1440/390 e light/dark. Somente depois registrar a confirmação técnica final de VC-02. Não iniciar VC-01 nem outra frente sem autorização.
